...
use crate::stream::{select_with_strategy, ClosedStreams, PollNext, SelectWithStrategy};
use core::pin::Pin;
...
pin_project! {
    ...
    pub struct Select<St1, St2> {
        #[pin]
        inner: SelectWithStrategy<St1, St2, fn(&mut PollNext)-> PollNext, PollNext, fn(ClosedStreams) -> bool>,
    }
}

#[inline]
fn exit_when_both_finished(closed_streams: ClosedStreams) -> bool {
...
#[inline]
fn exit_when_either_finished(closed_streams: ClosedStreams) -> bool {
    match closed_streams {
    ...
    St2: Stream<Item = St1::Item>,
{
    assert_stream::<St1::Item, _>(Select {
        inner: select_with_strategy(stream1, stream2, round_robin, exit_when_both_finished),
    })
}
...
{
    assert_stream::<St1::Item, _>(Select {
        inner: select_with_strategy(stream1, stream2, round_robin, exit_when_either_finished),
    })
}
